...
# Define a root for agent reports and backups
#
# The agent lives under ``brains/agent/service``.  The Maven project root is
# four levels up from this file (service -> agent -> brains -> maven).  Use this
# root to locate the global ``reports/agent`` directory for storing execution
# logs and backups.  If you relocate this file, adjust the ``parents`` index
# accordingly to continue pointing to the Maven root.
AGENT_REPORT_DIR = Path(__file__).resolve().parents[4] / "reports" / "agent"
...
def service_api(msg: Dict[str, Any]) -> Dict[str, Any]:
    """
    Entry point for the agent executor service.
    ...
    if op == "MINILOOP":
        ...
        if autotune:
            try:
                cog_root = Path(__file__).resolve().parents[4] / "brains" / "cognitive"
                brain_names = [
                    "sensorium",
                    "planner",
                    "language",
