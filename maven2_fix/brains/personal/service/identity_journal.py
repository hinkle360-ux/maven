...
def _snapshot_path() -> Path:
    ...
    base = Path(__file__).resolve().parents[2]  # brains/personal/service/ → brains/personal
    return base / "memory" / "identity_snapshot.json"
...
def _load_self_model() -> Dict[str, Any]:
    ...
    try:
        ...
        if cfg_path:
            ...
            base_root = Path(__file__).resolve().parents[2].parents[1]
            p = base_root / cfg_path
        else:
            # Default path relative to brains/personal/memory
            p = Path(__file__).resolve().parents[2] / "memory" / "self_model.json"
        if p.exists():
