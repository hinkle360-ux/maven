...
def _identity_snapshot_path() -> Path:
    ...
    """
    # parents[1] = brains/personal, so attach memory directory beneath it
    return Path(__file__).resolve().parents[1] / "memory" / "identity_snapshot.json"
...
def _ltm_path() -> Path:
    ...
    """
    return Path(__file__).resolve().parents[1] / "memory" / "ltm" / "self_factual_bank.json"
