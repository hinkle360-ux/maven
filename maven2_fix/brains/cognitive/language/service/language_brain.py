...
def _diag_log(tag, rec):
    try:
        root = Path(__file__).resolve().parents[4]  # .../maven2_fix/
...
def run_diagnostics():
    ...
    summ = root / "reports" / "diagnostics" / "summary.md"
...
# Load greeting profile from the language memory directory.  The file
# contains a JSON object mapping tone names to lists of strings.
try:
    _GREETING_PROFILE_PATH = Path(__file__).resolve().parent.parent / "memory" / "greeting_profile.json"
...
def _parse_intent(text: str) -> Dict[str, Any]:
    ...
    # facts.  A simple heuristic looks for a 'more' prefix or matches
    # predefined phrases.  Longer inputs beginning with "more " (two words)
    # are also considered follow‑ups.
    try:
        _cql = lower.strip()
    except Exception:
        _cql = ""
...
def service_api(msg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if op == "GENERATE_CANDIDATES":
        ...
        try:
            ...
            if _utterance in {"diag", "diagnose", "run diagnostics"}:
                ...
                fails, snap, diag_ctx = [], {}, {}
                # clear previous diag file each run
                try:
                    root = Path(__file__).resolve().parents[4]  # .../maven2_fix/
                    diag_file = root / "reports" / "diagnostics" / "diag.jsonl"
                    if diag_file.exists():
                        diag_file.unlink()
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                msg_text = "Diagnostics: OK" if not fails else f"Diagnostics: {len(fails)} failure(s). See reports/diagnostics/summary.md"
                return {
                    "ok": True,
                    "op": op,
                    "mid": mid,
        ...
        try:
            ...
            if ("what color do i like" in _utterance) or ("favorite color" in _utterance) or \
               ("color" in _utterance and "like" in _utterance):
        ...
        try:
            if any(_rp in low_txt_rel for _rp in relation_patterns):
                ...
                # Load self‑model to insert agent identity into the response.  Default to "Maven" if unavailable.
                name = "Maven"
                try:
                    from pathlib import Path
                    import json as _json
                    # Determine project root and locate self_model.json
                    root = Path(__file__).resolve().parents[4]
                    sm_path = root / "brains" / "personal" / "memory" / "self_model.json"
                    if sm_path.exists():
                        try:
                            sm_data = _json.loads(sm_path.read_text(encoding="utf-8"))
            ...
            if any(p in low_txt for p in env_patterns):
                ...
                if env_api:
                    try:
                        ...
                        pass
                # Load the self model's name to personalise the reply
                name = "Maven"
                try:
                    from pathlib import Path as _Path
                    import json as _json
                    root_path = _Path(__file__).resolve().parents[4]
                    sm_path = root_path / "brains" / "personal" / "memory" / "self_model.json"
                    if sm_path.exists():
                        try:
                            sm_data = _json.loads(sm_path.read_text(encoding="utf-8"))
                            name = str(sm_data.get("name") or name)
        ...
        try:
            ...
            if _math_key:
                # Parse the expression to compute the result
                try:
                    result = str(eval(_math_key))
                except Exception:
                    result = None
        ...
        try:
            ...
            if intent3 == "greeting":
                ...
                try:
                    ...
                    pass
                # Personalise with the agent's name from the self model, if available.
                try:
                    from pathlib import Path as _Path
                    import json as _json
                    # Resolve the project root and locate the self model file
                    root_path = _Path(__file__).resolve().parents[4]
                    sm_path = root_path / "brains" / "personal" / "memory" / "self_model.json"
        ...
        if is_q:
            ...
            if storable_type == "PREFERENCE":
                ...
                if not subject:
                    try:
                        ...
                        subject = None
                if not subject:
                    # Cannot determine subject; fallback to generic acknowledgement
                    cand = {
            ...
            # Speculation or theory → thoughtful acknowledgements
                ...
                if not peer_matched:
                    # For non‑storable input we should not imply that the information has been stored.
                    # Use a neutral acknowledgement that avoids references to memory or storage.
                    candidates.append({
                ...
                # Fallback.  Provide a neutral acknowledgement instead of a
                # filler phrase when we cannot determine how to respond.  This
                # avoids repeating meta statements like "I received that
                # information." which can be confusing.  See issue
                # #context_snapshot for examples.
                candidates.append({
                    "type": "acknowledgement",
                    "text": "Got it — noted.",
                    "confidence": 0.25,
    ...
    if op == "FINALIZE":
        ...
        try:
            ...
            verbosity = 1.0
        # Allow the user profile to override tone and verbosity preferences.  The user
        # can specify attributes like "tone" (e.g. "formal", "casual") and
        # "verbosity" (e.g. numeric or named levels) in their profile.  These
        # settings take precedence over the personality snapshot.
        try:
        ...
        if user_profile:
            try:
                ...
                prof = {}
            # Override tone if specified in profile
            prof_tone = None
            ...
            if prof_tone:
                tone = prof_tone
            # Override verbosity if specified in profile
            prof_verb = None
            ...
            if prof_verb:
                ...
                if prof_verb in mapping:
                    verbosity = mapping[prof_verb]
                else:
                    # Attempt to parse numeric string
                    try:
                        verbosity = float(prof_verb)
                    except Exception:
                        verbosity = verbosity
        ...
        try:
            ...
            if st_type_local == "QUESTION":
                ...
                if not any(p in ans_lower for p in skip_phrases):
                    # Compute the Maven root by ascending from this file.  The
                    # hierarchy is .../language/service/language_brain.py -> language
                    # -> cognitive -> brains -> maven.  parents[4] yields
                    # maven_unzip/maven.
                    root = Path(__file__).resolve().parents[4]
                    qa_dir = root / "reports"
                    qa_dir.mkdir(parents=True, exist_ok=True)
                    qa_file = qa_dir / "qa_memory.jsonl"
                    repair_file = qa_dir / "self_repair.jsonl"
                    # Check for existing answer to log contradictions
                    try:
                        if qa_file.exists():
                            with open(qa_file, "r", encoding="utf-8") as qfh:
                                for line in qfh:
                                    try:
                                        rec = json.loads(line)
                                    except Exception:
                                    ...
                                    if not isinstance(rec, dict):
                                        continue
                                    if str(rec.get("question", "")).strip().lower() == text.strip().lower():
                                        old_ans = str(rec.get("answer", "")).strip()
                                        if old_ans and old_ans.lower() != ans_lower:
                                            # Found contradictory answer; log to self_repair
                                            try:
                                                with open(repair_file, "a", encoding="utf-8") as rf:
                                                    json.dump({
                                                        "question": text.strip(),
                                                        "old_answer": old_ans,
                        ...
                        pass
                    # Append new answer to qa_memory and update topic statistics
                    try:
                        with open(qa_file, "a", encoding="utf-8") as qfh:
                            json.dump({
                                "question": text.strip(),
    ...
    try:
        ...
        if st_type == "QUESTION" and (verdict8 in {"UNANSWERED", "UNKNOWN", ""}):
            return {
                ...
                "evidence": {"query": ctx.get("original_query")},
            }
